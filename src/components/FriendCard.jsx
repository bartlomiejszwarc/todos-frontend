...
function FriendCard({ friend }) {
  ...
  return (
    <div className='flex flex-row xl:flex-col items-center xl:justify-center xl:aspect-square xl:w-[20%] w-full border-2 rounded-lg m-1 lg:py-0 py-2'>
      <div className='leading-none flex flex-row xl:flex-col items-center xl:justify-center justify-between w-full px-3 xl:px-0 xl:space-y-4'>
        <div className='flex xl:flex-col items-center space-x-3 xl:space-x-0'>
          <CustomAvatar user={friend} />

          <div className='flex flex-col xl:items-center'>
            <span className='md:text-2xl font-bold text-center'>{friend.displayName} </span>
            <span className='text-neutral-500'>{friend.username}</span>
          </div>
        </div>
        <button
